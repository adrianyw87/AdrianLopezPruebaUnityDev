...
namespace KartGame.Track
{
    ...
    public class TimeDisplay : MonoBehaviour
    {
        ...
        [Tooltip("Adrian: objeto para la animación de BestLap")]
        public GameObject BestLapPanel;
        
        bool wasBestLap = false;
        ...
        void UpdateLap()
        {
            ...
            finishedLapTimes.Add(currentTime);
            currentLapStartTime = Time.time;
            ...
            //Adrian: almacenar la mejor vuelta current
            SetCurrentBestLapTime();

            //Adrian: compare historical best time with current for show the message           
            if (isBestLapTime())
            {                
                BestLapPanel.SetActive(true);
                BestLapPanel.GetComponent<PlayableDirector>().Play();
            }
        ...
        public float GetHistoricalBestLap()
        {
            float historicalBestLap = PlayerPrefs.GetFloat("bestLap_historical");
            return historicalBestLap;
        }

        public void SetCurrentBestLapTime()
        {
            //Adrian: Catching and setting current bestLap time
            float bestLap_current = finishedLapTimes[getBestLap()];            
            PlayerPrefs.SetFloat("bestLap_current", bestLap_current);            
        }

        public bool isBestLapTime()
        {
            float historicalBestTime = GetHistoricalBestLap();            
            float currentBestTime = PlayerPrefs.GetFloat("bestLap_current");

            if (currentBestTime < historicalBestTime)
            {
                PlayerPrefs.SetFloat("bestLap_historical", currentBestTime);
                wasBestLap = true;
                return true;
            }else if(historicalBestTime == 0)
            {
                PlayerPrefs.SetFloat("bestLap_historical", currentBestTime);               
            }            
            return false;
