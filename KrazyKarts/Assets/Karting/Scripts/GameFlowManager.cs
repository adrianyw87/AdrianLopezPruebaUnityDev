...
public class GameFlowManager : MonoBehaviour
{
    ...
    CoinsCounterManager coinsCountManager;
    ...
    {        
        ...
        coinsCountManager = FindObjectOfType<CoinsCounterManager>();
    ...
    void Update()
    {
        //Adrian: resetear las stats del jugador para facilitar el testeo
        if (Input.GetKeyDown(KeyCode.R))
        {
            PlayerPrefs.SetFloat("bestLap_historical", 0);
            PlayerPrefs.SetFloat("historicalCoins", 0);
            Debug.Log("reset player prefs");
        }
        if (gameState != GameState.Play)
        {
            elapsedTimeBeforeEndScene += Time.time;
            if(elapsedTimeBeforeEndScene >= endSceneLoadDelay)
            {
                ...
                {
                    //Adrian: cambiamos el antiguo sistema de cambio de escena por el de aparici�n de GameOverMenu
    ...
    void EndGame(bool win)
    {
        ...
        //Adrian: comento el anterior sistema de cambio de escena
        ...
        // Adrian: conservamos el cambio de gamestate a win y algunas instrucciones m�s que son necesarias de momento
        gameState = win ? GameState.Won : GameState.Lost;
        endGameFadeCanvasGroup.gameObject.SetActive(true);
        m_TimeLoadEndGameScene = Time.time + endSceneLoadDelay + delayBeforeFadeToBlack;        

        //// Adrian: sumar las monedas conseguidas en esta carrera al total hist�rico de monedas        
        int currentCoins = coinsCountManager.coins;
        int historicalCoins = PlayerPrefs.GetInt("historicalCoins");
        int totalAmount = currentCoins + historicalCoins;

        if (historicalCoins == 0)
        {
            PlayerPrefs.SetInt("historicalCoins", currentCoins);
        }
        else
        {
            PlayerPrefs.SetInt("historicalCoins", totalAmount);
        }
